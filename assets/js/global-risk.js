(function () {
  ...
  function updateDocumentMeta() {
    ...
    const title = `Global Risk Index Today — ${pageData.reportDate} | World Risk Report`;
    const description = `Global Risk Index for ${pageData.reportDate}: ${Number.isFinite(score) ? score + "/100" : "daily update"}. Daily conflict, economy, health, climate and country risk report from Pulse Of Globe.`;
  ...
  function renderSources() {
    const sourceList = $("daily-source-list");
    if (!sourceList) return;
    sourceList.innerHTML = toArray(pageData.sourceNotes).map(item => {
      const label = escapeHTML(item.label);
      const date = escapeHTML(item.date || "");
      const note = escapeHTML(item.note || "");
      const url = String(item.url || "");
      const safeUrl = /^https:\/\//.test(url) ? escapeHTML(url) : "";
      return `<article class="method-card"><h3>${label}</h3><p>${date ? `<strong>${date}</strong> · ` : ""}${note}</p>${safeUrl ? `<p><a href="${safeUrl}" target="_blank" rel="noopener noreferrer">Open original source →</a></p>` : ""}</article>`;
    }).join("");
  }
